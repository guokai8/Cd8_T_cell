...
###Choose pcs=11
###
sample.combined <- RunUMAP(sample.combined, reduction = "pca", dims = 1:13)
sample.combined <- FindNeighbors(sample.combined, reduction = "pca", dims = 1:13)
...
geom_bar(stat="identity")+geom_text_repel(aes(label = paste0(round(100*cell,1), "%")),position = position_stack())+
...
clusters<-c("CD8+ Tn","CD8+ Tem","2","CD8+ Tn",
            "CD8+ Tem","CD8+ Tn","CD8+ Tem","CD8+ Tcm",
            "8","9","10","CD8+ Tn",
            "CD8+ Tn","13","CD8+ Tcm")
...
#
meta%>%filter(celltype=="CD8+ Tem")%>%group_by(Time,seurat_clusters)%>%summarise(count=n())%>%mutate(cell=count/sum(count))%>%ggplot(aes(Time,cell,fill=seurat_clusters))+
geom_bar(stat="identity")+geom_text(aes(label = paste0(round(100*cell,1), "%")),position = position_stack())+
scale_fill_manual(values=pcols[c(2,4,6)])+theme_classic(base_size = 15)+xlab("")+ylab("")+labs(fill="cluster")
dev.print(pdf,file="TEM_pro.pdf")
##
...
Idents(samf)<-"cond"
deg7dm<-lapply(c(0,1,3,4,5,6,7,11,12,14), function(x)FindMarkers(sample.combined,ident.1 = paste("D7",x,sep="_"),ident.2=paste("Mock",x,sep="_"),test.use = "MAST",logfc.threshold = 0))
deg14dm<-lapply(c(0,1,3,4,5,6,7,11,12,14), function(x)FindMarkers(sample.combined,ident.1 = paste("D14",x,sep="_"),ident.2=paste("Mock",x,sep="_"),test.use = "MAST",logfc.threshold = 0))
deg14d7<-lapply(c(0,1,3,4,5,6,7,11,12,14), function(x)FindMarkers(sample.combined,ident.1 = paste("D14",x,sep="_"),ident.2=paste("D7",x,sep="_"),test.use = "MAST",logfc.threshold = 0))
### write out
deg7dms<-lapply(deg7dm, function(x)subset(x,p_val_adj<0.05&abs(avg_log2FC)>0.25))
deg14dms<-lapply(deg14dm, function(x)subset(x,p_val_adj<0.05&abs(avg_log2FC)>0.25))
deg14d7s<-lapply(deg14d7, function(x)subset(x,p_val_adj<0.05&abs(avg_log2FC)>0.25))
###                 
...
Heatmap(sc,features=paths,group_by=="seurat_clusters",cluster_rows=F,cluster_cols=F)
